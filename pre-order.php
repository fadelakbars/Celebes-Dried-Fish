...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_preorder'])) {
    ...
    if ($can_proceed) {
        ...
        if ($res->num_rows > 0) {
            ...
            $note = "Produk: " . $product['name'] . "\n" . $note;
        } else {
            $total_price = 0;
        }
        $stmt->close();

        $stmt = $conn->prepare("INSERT INTO pre_orders (user_id, name, phone, email, address, amount, total_price, date, payment, payment_proof, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("issssiissss", $user_id, $name, $phone, $email, $address, $amount, $total_price, $date, $payment, $payment_proof, $note);

        if ($stmt->execute()) {
            $order_id = $conn->insert_id;
            $success_msg = "Pre-Order berhasil dibuat! Anda akan dialihkan ke halaman pembayaran.";
            echo "<script>
                setTimeout(function() {
                    window.location.href = 'payment.php?type=pre_order&id=$order_id';
                }, 1500);
            </script>";
        } else {
            $error_msg = "Terjadi kesalahan. Gagal membuat Pre-Order.";
        }
        $stmt->close();
    }
}
...
$products_query = $conn->query("SELECT id, name, price, image FROM products ORDER BY name ASC");
$products = [];
...
<div style="background-color: var(--primary-light); padding: 25px 0; text-align: center;">
    <div class="container">
        <h1 style="color: var(--primary-color); margin: 0 0 5px 0;">Form Pre-Order</h1>
        <p>Pesan terlebih dahulu untuk tanggal yang Anda inginkan</p>
    </div>
</div>
...
<section style="padding: 60px 0;">
    <div class="container" style="max-width: 800px;">
        ...
        <div style="background: var(--white); padding: 30px; border-radius: 10px; box-shadow: 0 5px 20px rgba(0,0,0,0.05);">
            ...
            <form action="pre-order.php" method="POST" enctype="multipart/form-data">
                <?php if (!empty($addresses)): ?>
                ...
                <div class="input-group" style="margin-top: 5px;">
                    <label style="margin-bottom: 12px; display: block;">Pilih Produk</label>
                    <div id="product-picker" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 14px;">
                        <?php foreach ($products as $p): ?>
                            <div class="product-pick-card"
                                 ...
                                 style="border: 2px solid var(--gray-border); border-radius: 10px; overflow: hidden; cursor: pointer; transition: all 0.2s; background: #fff; text-align: center;">
                                <div style="height: 120px; overflow: hidden; background: #f9f3ef;">
                                    ...
                                    <?php else: ?>
                                        <div style="height:100%; display:flex; align-items:center; justify-content:center;">
                                            <i class="fas fa-fish" style="font-size: 2.5rem; color: var(--primary-light);"></i>
                                        </div>
                                    <?php endif; ?>
                                </div>
                                <div style="padding: 10px 8px;">
                                    <div style="font-weight: 600; font-size: 0.88rem; color: var(--text-dark); margin-bottom: 4px; line-height: 1.3;"><?php echo htmlspecialchars($p['name']); ?></div>
                                    <div style="font-size: 0.82rem; color: var(--primary-color); font-weight: 700;">Rp <?php echo number_format($p['price'], 0, ',', '.'); ?>/kg</div>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
                    <!-- Validation message -->
                    <p id="product-required-msg" style="color: #c0392b; font-size: 0.85rem; margin-top: 8px; display: none;"><i class="fas fa-exclamation-circle"></i> Pilih produk terlebih dahulu.</p>
                </div>
                ...
                <div class="input-group" style="margin-top: 20px;">
                    ...
                    <textarea id="note" name="note" rows="2" placeholder="Nama produk yang dipesan, detail acara, dll."></textarea>
                </div>

                <button type="submit" name="submit_preorder" class="btn btn-primary btn-block" style="margin-top: 20px;">Lanjut Pembayaran</button>
            </form>
        </div>
    </div>
</section>
...
<style>
    .product-pick-card:hover {
        ...
        box-shadow: 0 6px 18px rgba(163,78,0,0.15);
    }
    .product-pick-card.selected {
        border-color: var(--primary-color) !important;
        box-shadow: 0 0 0 3px rgba(163,78,0,0.18);
...
<script>
    function selectProduct(card) {
        ...
        document.getElementById('product-required-msg').style.display = 'none';
    }

    // Intercept form submit to validate product selection
    document.querySelector('form[action="pre-order.php"]').addEventListener('submit', function(e) {
        if (!document.getElementById('product_id').value) {
            e.preventDefault();
            document.getElementById('product-required-msg').style.display = 'block';
